...
public class Mami : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            if (!GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerMovement>().hasLifeSavior())
            {
                PlayerDeath();
            }
            else
    ...
    private IEnumerator Waiting()
    {
        ...
        GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerMovement>().RemoveLifeSavior();
        waiting = false;
    }
}
